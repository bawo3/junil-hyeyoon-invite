...
- **지도 미리보기(오시는 길 섹션)**: 좌표가 있고 카카오맵 JS SDK가 정상 로드되면 실제 카카오 지도(마커 포함)를, 좌표가 없거나 SDK 로드에 실패하면 Google Maps 임베드(`output=embed`, 키 불필요)로 자동 폴백. 지도 조작(드래그/줌)은 오버레이를 한 번 탭하기 전까지 잠겨있어 페이지 스크롤을 방해하지 않음
  - 카카오맵 JS SDK appkey는 카카오톡 공유용 `KAKAO_JS_KEY`와 **다른 값**(재현 청첩장 프로젝트의 앱 키를 재사용) — 그 앱 콘솔의 [플랫폼] Web에 배포 도메인이 등록되어 있어야 동작
